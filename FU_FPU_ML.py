...
def FU(iFu, option=3, degree=3):
    ...
    df['FU{}_Delta_racosdec'.format(iFu)] = deltara2sky(df['FU{}_Delta_ra'.format(iFu)], (df['FU{}_dec'.format(iFu)] + df['dec'])/2 )
    df['FU{}_Delta_X_calculated'.format(iFu)] = df['FU{}_X_calculated'.format(iFu)] - reference00[iFu][0]
    df['FU{}_Delta_Y_calculated'.format(iFu)] = df['FU{}_Y_calculated'.format(iFu)] - reference00[iFu][1]
    df['FU{}_Radius_calculated'.format(iFu)] = np.sqrt(df['FU{}_Delta_X_calculated'.format(iFu)]**2 + df['FU{}_Delta_Y_calculated'.format(iFu)]**2)
    rho, phi = cart2pol(df['FU{}_Delta_racosdec'.format(iFu)], df['FU{}_Delta_dec'.format(iFu)])
    df['FU{}_rho'.format(iFu)] = rho
    df['FU{}_phi'.format(iFu)] = phi
    ...
    cond = df.alt >= 25.0
    if option == 1:
        features = ('ha', 'dec', 'temperature', 'airmass', 'alt',
                    'FU{}_Delta_racosdec'.format(iFu), 'FU{}_Delta_dec'.format(iFu), 'FU{}_Delta_X_calculated'.format(iFu), 'FU{}_Delta_Y_calculated'.format(iFu), 'FU{}_Radius_calculated'.format(iFu), 'FU{}_Delta_Teff'.format(iFu),
                    )
    elif option == 2:
        features = ('ha', 'dec', 'temperature', 'airmass', 'alt',
                    'FU{}_Delta_racosdec'.format(iFu), 'FU{}_Delta_dec'.format(iFu),
                    'FU{}_X_calculated'.format(iFu), 'FU{}_Y_calculated'.format(iFu), 'FU{}_Delta_Teff'.format(iFu),
                    )
    elif option == 3:
        features = ('ha', 'dec', 'temperature', 'airmass', 'alt',
                    'FU{}_rho'.format(iFu), 'FU{}_phi'.format(iFu), 'FU{}_X_calculated'.format(iFu), 'FU{}_Y_calculated'.format(iFu), 'FU{}_Delta_Teff'.format(iFu),
                    )
    elif option == 4:
        features = ('ha', 'dec', 'temperature', 'airmass', 'alt',
                    'FU{}_rho'.format(iFu), 'FU{}_phi'.format(iFu), 'FU{}_Delta_Teff'.format(iFu),
                      )
    elif option == 5:
        features = ('ha', 'dec', 'temperature', 'airmass', 'alt',
                    'FU{}_rho'.format(iFu), 'FU{}_phi'.format(iFu),
                      )
    else:
        raise ValueError('option out of range')
    print('features : {}'.format(features))
    target = ('FU{}_dX'.format(iFu), 'FU{}_dY'.format(iFu))
    
    X = df.loc[cond, features]
    t = df.loc[cond, target]
    ...
    X_train = df.loc[cond_train, features]
    t_train = df.loc[cond_train, target]
    ...
    X_test = df.loc[cond_test, features]
    t_test = df.loc[cond_test, target]
    ...
    # train
    ocX = t_train['FU{}_dX'.format(iFu)] - y_train[:, 0]
    ocY = t_train['FU{}_dY'.format(iFu)] - y_train[:, 1]
    ocR = np.sqrt(ocX**2 + ocY**2)
    std = np.std(ocR)
    ...
    # test
    ocX = t_test['FU{}_dX'.format(iFu)] - y_test[:, 0]
    ocY = t_test['FU{}_dY'.format(iFu)] - y_test[:, 1]
    ocR = np.sqrt(ocX**2 + ocY**2)
    std = np.std(ocR)
